// index.js (版本 3.1.3 - SillyTavern 适配版)

import { registerExtension } from "../../extensions.js";

registerExtension("手机UI美化", {
    async init() {
        ...
        function processNewData() {
            ...
            while ((weatherMatch = weatherRegex.exec(sourceContent)) !== null) {
                ...
                DOMElements.sourceData.textContent =
                    DOMElements.sourceData.textContent.replace(weatherMatch[0], '');
            }
        ...
        function setupEventListeners() {
            DOMElements.phone.addEventListener('click', e => {
                ...
                if (target = C('#weather-widget')) modules.utils.navigateTo('weather-app-view');
                    ...
                    e.preventDefault();
                    modules.utils.navigateTo(target.dataset.app);
                }
            });
        ...
        function initialize() {
            ...
            modules.commandQueue.init({
                sendCommandToAI: modules.utils.sendCommandToAI,
                campusCardModule: modules.campusCard
            });
            modules.weather.init({ navigateTo: modules.utils.navigateTo });
            modules.campusCard.init({
                navigateTo: modules.utils.navigateTo,
                queueCommand: modules.commandQueue.queueCommand,
                showIslandNotification: modules.utils.showIslandNotification
            });
            modules.boliBite.init({
                navigateTo: modules.utils.navigateTo,
                queueCommand: modules.commandQueue.queueCommand,
                showIslandNotification: modules.utils.showIslandNotification,
                campusCardModule: modules.campusCard
            ...
            if (!loadState() || !state.config) {
                state = {
                    config: {}, chatList: [], chatLogs: {},
                    momentsLog: [], contacts: [],
                    contactMap: {}, friendRequests: [],
                    handledRequestIds: [], avatarOverrides: {},
                    lastProcessedSource: null
                };
                ['chatList','momentsLog','contacts','friendRequests','handledRequestIds']
                    .forEach(k => { if (!state[k]) state[k] = []; });
                ['chatLogs','contactMap','avatarOverrides']
                    .forEach(k => { if (!state[k]) state[k] = {}; });
            }
            ...
            processNewData();
            console.log("手机UI美化 插件初始化成功 ✅ (v3.1.3)");
        }
